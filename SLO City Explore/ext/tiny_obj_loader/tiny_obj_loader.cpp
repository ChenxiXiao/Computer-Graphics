...
updateVertex(std::map<vertex_index, unsigned int> &vertexCache,
             ...
             std::vector<float> &texcoords,std::vector<float> &colors,
  ...
  assert(in_positions.size() > static_cast<unsigned int>(6 * i.v_idx + 2));

  positions.push_back(in_positions[6 * static_cast<size_t>(i.v_idx) + 0]);
  positions.push_back(in_positions[6 * static_cast<size_t>(i.v_idx) + 1]);
  positions.push_back(in_positions[6 * static_cast<size_t>(i.v_idx) + 2]);
  colors.push_back(in_positions[6 * static_cast<size_t>(i.v_idx) + 3]);
  colors.push_back(in_positions[6 * static_cast<size_t>(i.v_idx) + 4]);
  colors.push_back(in_positions[6 * static_cast<size_t>(i.v_idx) + 5]);
  ...
  for (size_t i = 0; i < faceGroup.size(); i++) {
    ...
    for (size_t k = 2; k < npolys; k++) {
      ...
      unsigned int v0 = updateVertex(
          ...
          shape.mesh.texcoords, shape.mesh.colors, in_positions, in_normals, in_texcoords, i0);
      unsigned int v1 = updateVertex(
          vertexCache, shape.mesh.positions, shape.mesh.normals,
          shape.mesh.texcoords, shape.mesh.colors, in_positions, in_normals, in_texcoords, i1);
      unsigned int v2 = updateVertex(
          vertexCache, shape.mesh.positions, shape.mesh.normals,
          shape.mesh.texcoords,shape.mesh.colors, in_positions, in_normals, in_texcoords, i2);
...
bool LoadObj(std::vector<shape_t> &shapes, // [output]
  ...
  while (inStream.peek() != -1) {
    ...
    if (token[0] == 'v' && isSpace((token[1]))) {
        token += 2;
        float x, y, z, r, b, g;
        const bool found_color = parseVertexWithColor(x, y, z, r, b, g, token);
        v.push_back(x);
        v.push_back(y);
        v.push_back(z);
        if(found_color){
    ...
    if (token[0] == 'f' && isSpace((token[1]))) {
      ...
      while (!isNewLine(token[0])) {
        vertex_index vi =
            parseTriple(token, static_cast<int>(v.size() / 6), static_cast<int>(vn.size() / 3), static_cast<int>(vt.size() / 2));
