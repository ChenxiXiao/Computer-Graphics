...
public:
   ...
   // http://www-01.ibm.com/support/knowledgecenter/ssw_aix_61/com.ibm.aix.opengl/doc/openglrf/gluPerspective.htm%23b5c8872587rree
	...
	void render()
	{
        ...
        createTranslateMat(cubeTrans, -1, 0, -6);
        createScaleMat(cubeScale,0.4,2,1);
        ...
        //second -H
        mesh->draw(prog);
        createTranslateMat(cubeTrans, -0.7, 0,-6);
        multMat(M, cubeScale,cubeTrans);
        
        
        glUniformMatrix4fv(prog->getUniform("P"), 1, GL_FALSE, P);
        glUniformMatrix4fv(prog->getUniform("V"), 1, GL_FALSE, V);
        glUniformMatrix4fv(prog->getUniform("M"), 1, GL_FALSE, M);        mesh->draw(prog);
        ...
        createTranslateMat(cubeTrans, -0.7, -0.2,-6 );
        createScaleMat(cubeScale,1.0,0.2,1);
        
        float R[16] = {0},  M_[16] = {0};
        
        createRotateMatZ(R, -0.5);
        //createRotateMatY(R1, 0.01);
        
        multMat(M_, cubeTrans, cubeScale);
        multMat(M, R,M_);
        //createRotateMatZ(cubeRotate,-1 );
        //multMat(V_, V, cubeRotate);
